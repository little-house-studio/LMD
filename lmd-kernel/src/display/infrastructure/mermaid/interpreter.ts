/**
 * Mermaid flowchart interpreter for LMD.
 * Parses / serializes standard `flowchart` / `graph` blocks used inside `## Diagram`.
 */
import type {
  Direction,
  EdgeType,
...
import { normalizeEntityIdBase } from '../../../shared-kernel/identity';
...
/** Greedy wrap by CJK-aware units. Empty paragraphs are dropped, matching measure. */
export function wrapNodeText(text: string, capacity: number): string[] {
  ...
  for (const paragraph of text.replace(/\r\n/g, '\n').split('\n')) {
    ...
    for (const char of Array.from(paragraph)) {
      const unit = char.charCodeAt(0) > 255 ? 1.8 : 1;
      if (current && units + unit > cap) {
        lines.push(current);
        current = char;
        units = unit;
      } else {
...
export function layoutNodeContent(
  ...
  const titleSource = title.trim() ? title : '未命名内容';
  const rawTitle = title.split(/\r?\n/).filter(Boolean);
  const rawDesc = description.split(/\r?\n/).filter(Boolean);
  const widthSamples = [...(rawTitle.length > 0 ? rawTitle : [titleSource]), ...rawDesc];
  const maxUnits = Math.max(8, ...widthSamples.map(textUnits));
  const width = boxWidth ?? Math.max(156, Math.min(240, 62 + maxUnits * 9));
  const wrapCapacity = Math.max(10, Math.floor((width - 48) / 9));
  const titleLines = wrapNodeText(titleSource, wrapCapacity);
  const descriptionLines = wrapNodeText(description, wrapCapacity);
  const titleBandHeight = 18 + Math.max(1, titleLines.length) * 28 + (descriptionLines.length === 0 ? 10 : 0);
  const descriptionBandHeight = descriptionLines.length > 0
    ? 20 + descriptionLines.length * 22
    : 30;
  return {
    width,
    height: Math.max(98, Math.min(760, titleBandHeight + descriptionBandHeight + 18)),
    titleLines: titleLines.length > 0 ? titleLines : [titleSource],
    descriptionLines,
    titleBandHeight,
    descriptionBandHeight,
  };
}

export function measureNodeContentSize(title: string, description = '') {
  const layout = layoutNodeContent(title, description);
