/**
 * Protocol-level entity IDs: `{title_base}_{short_hash}`.
 * Stable across title-preserving renames when a code suffix already exists.
 */
...
export function isStableEntityId(value: string) {
  return ENTITY_ID_WITH_CODE_PATTERN.test(value);
}
